...
that can be read and written using XML and Alembic .abc files.
"""
...
log = logging.getLogger("scenegraphXML")
log.setLevel(SG_XML_LOG_LEVEL)
...
def createScenegraphElementFromXMLData(xmlElement):
    ...
    raise ValueError(
        'No valid scenegraph element found when reading XML data for ScenegraphElement')
...
def applyXformToVector(m, v):
    ...
    """
    # matrix multiplication based on homogeneous vectors of form (x, y, z, 1)
    ...
    # trap for case w == 0
    if w == 0:
        raise ValueError("w==0 after homogeneous matrix multiplication")
    else:
        return [x/w, y/w, z/w]
...
def applyXformToBounds(curXform, curBounds):
    ...
    计算Xform(16个值的列表)对界限(6个值的列表)的影响的实用函数
    """
    newBounds = None
    ...
    for ix, iy, iz in ((0, 2, 4), (1, 2, 4), (0, 3, 4), (1, 3, 4), (0, 2, 5), (1, 2, 5), (0, 3, 5), (1, 3, 5)):
        ...
        curPos = applyXformToVector(
            curXform, [curBounds[ix], curBounds[iy], curBounds[iz]])
        # if this is the first corner, initialize the bounds
        if newBounds is None:
            newBounds = [curPos[0], curPos[0], curPos[1],
                         curPos[1], curPos[2], curPos[2]]
...
class ArbitraryAttribute:
    ...
    def writeXMLData(self, xmlArbitraryAttribute):
        ...
        using ElementTree.
        """
        log.debug('    calling ArbitraryAttribute.writeXMLData')
        ...
        if self.value is None and self.channelIndex is None:
            raise ValueError(
                'Neither value nor channelIndex set for ArbitraryAttribute')

        if self.value is not None:
            if self.dataType == 'float':
                xmlArbitraryAttribute.attrib['value'] = str(float(self.value))
            elif self.dataType == 'floatList':
                xmlArbitraryAttribute.attrib['value'] = ' '.join(
                    str(float(x)) for x in self.value)
        ...
        if self.channelIndex is not None:
            xmlArbitraryAttribute.attrib['channelIndex'] = str(
                self.channelIndex)
    ...
    def readXMLData(self, xmlArbitraryAttribute):
        ...
        using ElementTree.
        """
        log.debug('    calling ArbitraryAttribute.readXMLData')
        ...
        self.channelIndex = intOrNone(
            xmlArbitraryAttribute.get('channelIndex'))
...
class Bounds:
    ...
    类来保存边界框值. 它还使用ElementTree编写和读取这些值在内存中的XML表示
    """
    ...
    def writeXMLData(self, xmlBounds):
        ...
        using ElementTree.
        使用ElementTree将边界框值写入内存中的XML表示。
        """
        log.debug('    calling Bounds.writeXMLData')
        if self.value is None and self.channelIndex is None:
            raise ValueError(
                'Bounds not set using static values or channelIndex')
    ...
    def readXMLData(self, xmlBounds):
        ...
        using ElementTree.
        """
        log.debug('    calling Bounds.readXMLData')
...
class Xform:
    ...
    def writeXMLData(self, xmlXform):
        ...
        using ElementTree.
        """
        log.debug('    calling Xfrom.writeXMLData')
        if self.value is None and self.channelIndex is None:
            raise ValueError('Neither value nor channelIndex set for Xform')

        if self.value is not None:
            xmlXform.attrib['value'] = ' '.join(
                str(float(x)) for x in self.value)
    ...
    def readXMLData(self, xmlXform):
        ...
        using ElementTree.
        """
        log.debug('    calling Xform.readXMLData')
...
class Proxy:
    ...
    def writeXMLData(self, xmlProxy):
        ...
        ElementTree.
        """
        log.debug('    calling Proxy.writeXMLData')
        if self.name is None:
            raise ValueError(
                'name is not set when writing out Proxy data to XML')
        xmlProxy.attrib['name'] = self.name

        if self.ref is None:
            raise ValueError(
                'ref is not set when writing out Proxy data to XML')
    ...
    def readXMLData(self, xmlProxy):
        ...
        ElementTree.
        """
        log.debug('    calling Proxy.readXMLData')
        self.name = xmlProxy.get('name')
        if self.name is None:
            raise ValueError(
                'Cannot find XML attribute "name" when reading XML data for Proxy')

        self.ref = xmlProxy.get('ref')
        if self.ref is None:
            raise ValueError(
                'Cannot find XML attribute "ref" when reading XML data for Proxy')
...
class LodData:
    ...
    def writeXMLData(self, xmlLodData):
        ...
        using ElementTree.
        """
        log.debug('    calling LodData.writeXMLData')
        if self.tag is None and self.weight is None:
            raise ValueError(
                'Neither tag nor weight set when writing out LodData to XML')
    ...
    def readXMLData(self, xmlLodData):
        ...
        using ElementTree.
        """
        log.debug('    calling LodData.readXMLData')
...
class LookFile:
    ...
    def writeXMLData(self, xmlLookFile):
        log.debug('    calling LookFile.writeXMLData')
    ...
    def readXMLData(self, xmlLookFile):
        log.debug('    calling LookFile.readXMLData')
...
class AttributeFile:
    ...
    def writeXMLData(self, xmlAttributeFile):
        log.debug('    calling AttributeFile.writeXMLData')
        if self.ref is not None:
    ...
    def readXMLData(self, xmlAttributeFile):
        log.debug('    calling AttributeFile.readXMLData')
...
class ChannelData:
    ...
    用于保存动画Channel数据的类。 它还使用ElementTree写入和读取这些值的内存XML表示形式。
    """
    ...
    def setValue(self, index, value):
        ...
        在给定的通道索引中添加一个值。 
        如果当前索引数小于指定的索引数，
        则将0.0添加到当前最后一个索引与要添加的索引之间的所有缺失索引。
        """
    ...
    def writeXMLData(self, xmlChannelData):
        ...
        using ElementTree.
        使用ElementTree将Channel Data值写入内存中的XML表示形式。
        """
        log.debug('    XXXXXXXX calling ChannelData.writeXMLData')
        if self.startFrame is None:
            raise ValueError(
                'startFrame not set when writing ChannelData to XML')
        xmlChannelData.attrib['startFrame'] = str(self.startFrame)

        if self.endFrame is None:
            raise ValueError(
                'endFrame not set when writing ChannelData to XML')
        xmlChannelData.attrib['endFrame'] = str(self.endFrame)

        if self.ref is not None:
            if self.relativeMode:
                # relPath = self.ref[self.ref.rfind('/')+1:]  # 源码
    ...
    def readXMLData(self, xmlChannelData):
        ...
        using ElementTree.
        使用ElementTree从内存中的XML表示中读取Channel Data值.
        """
        log.debug('    calling ChannelData.readXMLData')
        self.startFrame = int(xmlChannelData.get('startFrame'))
        self.endFrame = int(xmlChannelData.get('endFrame'))
    ...
    def writeXMLChannelFile(self, frameNumber, verbose=True):
        ...
        使用ElemenTtree将内存中的XML表示形式写入通道xml文件中。
        在使用4个零填充的.xml扩展名之前，将在文件名中使用帧号。
        """
    ...
    def readXMLChannelFile(self, frameNumber):
        ...
        padding.
        使用ElemenTree将通道xml文件的内容读取到内存中的XML表示形式中。
        在使用4个零填充的.xml扩展之前，将在构建文件名时使用frameNumber。
        """
        filepath = self.ref + ".chan.%04d.xml" % frameNumber
...
class ScenegraphElement:
    ...
    # Group，ScenegraphRoot和Reference类使用的基类
    """
    ...
    def setAttributeFile(self, ref=None, groupName=None, customParser=None, channelIndex=None):
        self.attributeFile = AttributeFile(
            ref, groupName, customParser, channelIndex)
    ...
    def setBounds(self, minx=None, maxx=None, miny=None, maxy=None, minz=None, maxz=None, value=None, channelIndex=None):
        ...
        """
        self.bounds = Bounds(minx, maxx, miny, maxy, minz,
                             maxz, value, channelIndex)
    ...
    def addArbitraryAttribute(self, name, dataType, value=None, numValues=None, channelIndex=None):
        ...
        """
        newArbitraryAttribute = ArbitraryAttribute(
            name, dataType, value, numValues, channelIndex)
    ...
    def writeXMLCommonData(self, xmlElement):
        ...
        使用ElementTree编写整个场景的内存XML表示形式
        """
        log.debug(
            '  calling ScenegraphElement.writeXMLCommonData for %s' % self.name)
    ...
    def readXMLCommonData(self, xmlElement):
        ...
        使用ElementTree读取整个场景的内存XML表示形式
        """
        self.name = xmlElement.get('name')
        log.debug(
            '  calling ScenegraphElement.readXMLCommonData for %s' % self.name)
...
class Group(ScenegraphElement):
    ...
    def __init__(self, name=None, instanceList=None, groupType=None, xform=None, bounds=None, proxyList=None, arbitraryList=None, lodData=None):
        ScenegraphElement.__init__(self, name=name, xform=xform, bounds=bounds,
                                   proxyList=proxyList, arbitraryList=arbitraryList, lodData=lodData)
            ...
            for curInstance in self.instanceList:
                ...
                if curInstanceBounds is not None:
                    ...
                    if curInstanceXform is not None:
                        curInstanceBounds = applyXformToBounds(
                            curInstanceXform, curInstanceBounds)
    ...
    def writeXMLData(self, sgElement, xmlElement, channelMapping=None, channelData=None):
        if channelMapping is not None:
            if sgElement in channelMapping.keys():
                ...
                log.debug('Missing [%s]' % sgElement.name)

        log.debug('calling Group.writeXMLData for %s' % self.name)
        ...
        if self.instanceList is not None:
            for curInstance in self.instanceList:
                ...
                curInstance.writeXMLData(
                    curInstance, xmlCurInstance, channelMapping, channelData)

    def readXMLData(self, xmlElement):
        log.debug('calling Group.readXMLData')
        ...
        if xmlInstanceList is None:
            raise ValueError(
                'Cannot find XML element "instanceList" when reading XML data for Group')
        self.instanceList = []
...
class Reference(ScenegraphElement):
    ...
    def __init__(self, name=None, refFile=None, refType='xml', xform=None, bounds=None, proxyList=None,
        ...
        ScenegraphElement.__init__(self, name=name, xform=xform, bounds=bounds, proxyList=proxyList,
                                   arbitraryList=arbitraryList, lodData=lodData, lookFile=lookFile, attributeFile=attributeFile)
    ...
    def writeXMLData(self, sgElement, xmlElement, channelMapping=None, channelData=None):
        ...
        log.debug('calling Reference.writeXMLData for %s' % self.name)
        self.writeXMLCommonData(xmlElement)
        ...
        if self.refFile is not None:
            ...
            raise ValueError(
                'refFile not set when writing out XML for Reference')

        if self.groupType is not None:
            xmlElement.attrib['groupType'] = self.groupType

    def readXMLData(self, xmlElement):
        log.debug('calling Reference.readXMLData')
        self.readXMLCommonData(xmlElement)
        ...
        if self.refFile is None:
            raise ValueError(
                'Cannot find XML attribute "refFile" when reading XMl data for Reference')
...
class ScenegraphRoot(Group):
    ...
    def writeXMLFile(self, filepath, verbose=True):
        ...
        if self.instanceList is None:
            raise ValueError(
                'instanceList not set when writing out XMl for ScenegraphRoot')
        ...
        for curInstance in self.instanceList:
            xmlCurInstance = ET.SubElement(xmlInstanceList, 'instance')
            curInstance.writeXMLData(
                curInstance, xmlCurInstance, channelMappingPackage, self.channelData)
    ...
    def readXMLFile(self, filepath):
        ...
        if xmlInstanceList is None:
            raise ValueError(
                'Cannot find XML element "instanceList" when reading XML data for ScenegraphRoot')
        self.instanceList = []
        for xmlCurInstance in xmlInstanceList:
            curInstance = createScenegraphElementFromXMLData(xmlCurInstance)
            self.instanceList.append(curInstance)
...
if __name__ == '__main__':
    ...
    #geometry1 = Reference('gnomeGeo', refType='abc', refFile='Importomatic/gnome.abc')
    geometry1 = Reference('marketGeo', refType='abc',
                          refFile='Marketplace/Market.abc')
    ...
    # rootGroup.setLookFile('Importomatic/10818_GnomeMaterial.klf')

    #channelData = ChannelData(startFrame=1, endFrame=10, ref='Importomatic/ChannelDataLook')
    #channelData = ChannelData(startFrame=1, endFrame=1, ref='Marketplace/ChannelData')
    root1 = ScenegraphRoot()
    #root1 = ScenegraphRoot(channelData=channelData)
    root1.addInstance(rootGroup)

    # root1.writeXMLFile('Importomatic/SG_Look.xml')
    root1.writeXMLFile('Marketplace/Market.xml')
